...
export default function App() {
  ...
  const initialMode = searchParams.get('mode') || 'mode-selection';

  // Current application mode: 'mode-selection' (initial) | 'simple' | 'complex'
  const [mode, setMode] = useState(initialMode);

  // Simple Mode Intro Assembly State
  const [showSimpleIntro, setShowSimpleIntro] = useState(searchParams.get('intro') === 'true');
  ...
  useEffect(() => {
    if (searchParams.get('demo') === 'true') {
      ...
      setIsComplete(true);
      setTotalSteps(15);
      setActiveStepNumber(15);
      setCurrentStatus('ANALYSIS COMPLETE');
      setVisibleSteps([
        { stepNumber: 1, title: 'Syntactic Axiom Normalization', formula: 'S = \\langle 25 \\times 4 \\rangle \\in \\mathcal{A}^*', explanation: 'Deconstructing the string into a deterministic context-free grammar generated by Dirichlet-Hamilton algebras.' },
        { stepNumber: 2, title: 'Gaussian Integral Decomposition', formula: '\\int_{-\\infty}^{\\infty} e^{-\\pi t^2} \\, dt = 1', explanation: 'Expressing unit scalar coefficients as the normalization factor of a Gaussian probability density distribution.' },
        { stepNumber: 3, title: 'Birkhoff Ergodic Flow Trajectory', formula: '\\lim_{N \\to \\infty} \\frac{1}{N} \\sum_{n=0}^{N-1} f(T^n x) = \\int_X f \\, d\\mu', explanation: 'Verifying that time average along calculation phase orbit matches spatial invariant measure.' },
        { stepNumber: 4, title: 'Riemann Zeta Analytic Continuation', formula: '\\zeta(s) = 2^s \\pi^{s-1} \\sin\\left(\\frac{\\pi s}{2}\\right) \\Gamma(1-s) \\zeta(1-s)', explanation: 'Regularizing the arithmetic summation lattice against critical line zeroes.' },
        { stepNumber: 15, title: 'Terminal Asymptotic Crystallization', formula: '\\lim_{k \\to \\infty} \\Phi_k(\\langle 25 \\times 4 \\rangle) \\longrightarrow 97', explanation: 'All 14 analytical dimensions have reached immutable equilibrium. Crystallizing into a definitive outcome.' }
      ]);
    }
    return () => {
